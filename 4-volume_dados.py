...
df_compras = pd.read_csv(f'{caminho_datasets}/compras.csv' , sep=';' , decimal=',', parse_dates=True ,index_col=0)
...
data_inicial = st.sidebar.date_input("Data Inicial", data_default - timedelta(days=6))
data_final = st.sidebar.date_input("Data Final", data_default)

df_compras_filter = df_compras[(df_compras.index.date >= data_inicial) & (df_compras.index.date < data_final +timedelta(days=1))]
...
valor_compra = f"R$ {valor_compras:.2f}"
col1.metric("valor de compra no periodo ", valor_compra)
...
col21.metric("Valor de compras no preíodo" , valor_compras_loja)
...
col31.metric("Valor de compras no periodo", valor_compras_vendedor)
col32.metric("Comissao no Periodo", valor_comissao_vendedor)
